...
clients_connection = {12: None, 23: None, 24: None}
...
def receive_from_server():
    while True:
        ...
        if data == "start":
            ...
            th = threading.Thread(
                target=send_chunks, args=(clients_connection[12], "back")
            )
            th.start()
            threads.append(th)
...
def receive_from_client(index, conn):
    ...
    while count < chunks_length:
        ...
        if client_index == "A" and seq >= chunks_length//2:
            clients_connection[23].send(get_seq_chunk("A", sequence_size, seq, content))
            clients_connection[24].send(get_seq_chunk("A", sequence_size, seq, content))
...
if __name__ == "__main__":
    ...
    clients_connection[23].close()
    clients_connection[24].close()
